...
class Solution:
    ...
    def maxSubArray(self, nums: List[int]) -> int:
        ...
        if len(nums) == 1:
            max_sum = nums
        for index_start in range(0, len(nums)):
            for index_last in range(index_start, len(nums)):
                # print("nums: {} = {}".format(''.join(str(max_sum)),str(sum(max_sum))))
                if sum(max_sum) < sum(nums[index_start:index_last+1]):
                    max_sum = nums[index_start:index_last+1]
                    # print("nums: {} = {}".format(''.join(str(max_sum)),str(sum(max_sum))))


        # print("best nums: {} = {}".format(''.join(str(max_sum)),str(sum(max_sum))))
        # print("Answer: {}".format(str(sum(max_sum))))
        ...
        #     else:
        #         for x, new_num in enumerate(sorted_arr):
        #             # 1, 2, 3
        #             # 0 < 1
        #             if len(sorted_arr) == 1:
        #                 if (num >= sorted_arr[x]):
        ...
        #                 else:
        #                     sorted_arr = [num] + sorted_arr
        #             elif x == len(sorted_arr)-1:
        #                     sorted_arr = sorted_arr + [num]
        #             else:
        #                 if (num < sorted_arr[x]):
        #                     sorted_arr = [num] + sorted_arr
        #                     break
        #                 elif (num >= sorted_arr[x]) & (num < sorted_arr[x+1]):
        #                     sorted_arr = sorted_arr[:x] + [num] + sorted_arr[x+1:]
        #                     break


        #             print('new sorted {}'.format(sorted_arr))

        # now go through and sum a sub array after the sorted list
        # sum_vals = 0
        # for i in range(len(sorted_arr)-1,-1,-1):
        #     print(sum(sorted_arr[i:]))
        #     if sum_vals <= sum(sorted_arr[i:]):
        #         sum_vals = sum(sorted_arr[i:])
        #     else:
        #         break

        # print('Final sum: {}'.format(sum_vals))
        # return sum_vals 
